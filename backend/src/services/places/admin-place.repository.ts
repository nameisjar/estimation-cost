...
const selectColumns = `id, external_place_id, name, category, address,
...
export class AdminPlaceRepository {
  ...
  async list(query: string, status: 'all' | 'active' | 'inactive', page: number, limit: number): Promise<AdminPlaceList> {
    const result = await this.pool.query<AdminPlaceRow>(
      `SELECT ${selectColumns}, count(*) OVER() AS total_count
        ...
        LIMIT $3 OFFSET $4`,
      [query, status, limit, (page - 1) * limit],
    );
    return {
  ...
  async stats(): Promise<AdminPlaceStats> {
    const result = await this.pool.query<{
      total: number | string; active: number | string; inactive: number | string; missing_address: number | string;
               ...
               count(*) FILTER (WHERE address = 'Alamat belum tersedia' OR trim(address) = '') AS missing_address
          FROM places`);
  ...
  async create(place: AdminPlaceInput): Promise<AdminPlace> {
    ...
    const result = await this.pool.query<{ id: string }>(
      ...
      [place.name, place.category, place.address || 'Alamat belum tersedia', place.lat, place.lng,
        ...
        place.searchArea || null, place.active, rank.popularity, rank.minZoom, rank.labelPriority],
    );
  ...
  async update(id: string, place: AdminPlaceInput): Promise<AdminPlace> {
    ...
    const result = await this.pool.query(
      ...
      [place.name, place.category, place.address || 'Alamat belum tersedia', place.lat, place.lng,
        place.rating ?? null, place.reviewCount ?? null, place.phone || null, place.website || null,
        place.openingHours || null, place.googleMapsUrl || null, place.searchKeyword || null,
        place.searchArea || null, place.active, rank.popularity, rank.minZoom, rank.labelPriority, id],
    );
