...
export function createApp(
  ...
  const handleError: ErrorRequestHandler = (error, _req, res, _next) => {
    ...
    if (bodyError.type === 'entity.parse.failed' || bodyError.type === 'entity.too.large') {
      res.status(bodyError.type === 'entity.too.large' ? 413 : 400).json({ success: false, error: { code: 'INVALID_BODY', message: 'Request harus berupa JSON valid dengan ukuran maksimal 8 KB.' } }); return;
    }
